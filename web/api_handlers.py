...
from utils.common import (
    api_error_handler, create_basic_script, ensure_user_uploads_dir,
    get_file_extension_from_url, generate_unique_filename, get_request_data,
    create_success_response, create_error_response, replace_paths_with_placeholders,
    set_absolute_paths_in_project
)
from service.draft_service import DraftService
from handlers.doc_handler import show_documentation, get_health_check
...
def api_video_segment():
    """视频片段处理"""
    data = get_request_data()
    result = DraftService.create_video_segment(
...
def api_download_from_url():
    ...
    # 生成文件名并保存
    filename = generate_unique_filename(file_type, url) + get_file_extension_from_url(url, content_type)
